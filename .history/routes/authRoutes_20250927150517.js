...
router.post("/edit", authMiddleware, async function (req, res) {
  const { name, email, specification, photo, occupation, description } = req.body;

  try {
    // Update current logged-in user
    const updatedUser = await userModel.findByIdAndUpdate(
      req.user.id, // from authMiddleware
      { name, email, specification, photo, occupation, description },
      { new: true } // return the updated document
    );

    console.log("Profile update successful", updatedUser.name);
    res.redirect("/dashboard"); // redirect user to dashboard after update
  } catch (error) {
