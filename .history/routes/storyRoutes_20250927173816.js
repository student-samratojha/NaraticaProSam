...
router.get("/", authMiddleware, async function (req, res) {
  ...
  res.render("stories", { user: req.user, stories });
});

router.get("/admin/:id", authMiddleware, adminMiddleware, async function (req, res) {
  const stories = await storyModel.find({ writer: req.params.id })
    .populate("writer", "name");
  res.render("stories", { user: req.user, stories });
});
